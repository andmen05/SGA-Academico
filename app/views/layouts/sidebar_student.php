...
<head>
    ...
    <title><?= $pageTitle ?? 'Portal Estudiantil | SGA' ?></title>
    ...
    <style>
        *,body{font-family:'Inter',sans-serif}
        ...
        .sbar-user-program{color:rgba(148,163,184,0.45);font-size:.58rem;margin-top:2px;line-height:1.3}
        .sbar-logout{
        ...
        .inp{border:1px solid #e2e8f0;border-radius:10px;padding:9px 14px;font-size:.8125rem;color:#1e293b;outline:none;transition:border-color .15s,box-shadow .15s;background:#fff;font-family:'Inter',sans-serif;width:100%;box-sizing:border-box}
        ...
        .tbl th{font-size:.65rem;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.09em;background:linear-gradient(135deg,#fafafa,#f8fafc);padding:11px 18px;border-bottom:1px solid #f1f5f9;text-align:left}
        ...
        .data-row{display:flex;justify-content:space-between;align-items:flex-start;font-size:.8rem;padding:4px 0}
        .data-row-label{color:#94a3b8;flex-shrink:0}
        .data-row-val{color:#1e293b;font-weight:500;text-align:right;max-width:60%}

        
        .fl{font-size:.65rem;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.09em;display:block;margin-bottom:6px}
        .fv{font-size:.8125rem;color:#1e293b;font-weight:500}

        
        .tip{font-size:.78rem;color:#64748b;padding:3px 0;padding-left:14px;position:relative}
        .tip::before{content:'';position:absolute;left:0;top:50%;transform:translateY(-50%);width:5px;height:5px;border-radius:50%;background:#b91c1c}

        
        .link-d{font-size:.78rem;color:#b91c1c;font-weight:600;text-decoration:none}
        .link-d:hover{color:#991b1b}

        
        .modal-ov{position:fixed;inset:0;background:rgba(0,0,0,.5);backdrop-filter:blur(5px);display:flex;align-items:center;justify-content:center;z-index:50;padding:16px}
        .modal-box{background:#fff;border-radius:20px;width:100%;max-width:530px;box-shadow:0 30px 80px rgba(0,0,0,.2);overflow:hidden;max-height:90vh;overflow-y:auto}
        .modal-hdr{background:linear-gradient(135deg,#0f172a,#1e293b);color:#fff;padding:20px 24px;display:flex;align-items:flex-start;justify-content:space-between;position:sticky;top:0}
        .modal-body{padding:24px}
        .desc-box{background:#f8fafc;border:1px solid #f1f5f9;border-radius:10px;padding:14px;font-size:.8rem;color:#475569;line-height:1.7}
        .resp-box{background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:14px;font-size:.8rem;color:#1e293b;line-height:1.7}
...
<aside class="sbar">
    ...
    <div class="sbar-logo-wrap">
        ...
        <div class="sbar-logo-sub">Portal Estudiantil</div>
    </div>
    ...
    <nav class="sbar-nav">
        ...
        <a href="/student/dashboard.php" class="sbar-link <?= $cur==='dashboard.php'?'active':'' ?>">
        ...
        <div class="sbar-section-title">Solicitudes</div>

        <a href="/student/nueva-solicitud.php" class="sbar-link <?= $cur==='nueva-solicitud.php'?'active':'' ?>">
            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <a href="/student/mis-solicitudes.php" class="sbar-link <?= $cur==='mis-solicitudes.php'?'active':'' ?>">
            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="sbar-user">
        <div class="sbar-user-row">
            ...
            <div>
                <div class="sbar-user-name"><?= htmlspecialchars($_SESSION['user']['nombre'].' '.$_SESSION['user']['apellido']) ?></div>
                <div class="sbar-user-role">Estudiante — Sem. <?= $_SESSION['user']['semestre'] ?></div>
                <div class="sbar-user-program"><?= htmlspecialchars($_SESSION['user']['programa']) ?></div>
            </div>
        </div>
...
<div class="mob-bar">
    <div style="display:flex;align-items:center;gap:10px">
        ...
        <span style="color:#fff;font-size:.85rem;font-weight:700">SGA Estudiante</span>
    </div>
...
<div id="mob-overlay" class="mob-menu" style="display:none" onclick="if(event.target===this)this.style.display='none'">
    <div class="mob-panel">
        ...
        <nav style="padding:12px 10px">
            <a href="/student/dashboard.php" class="sbar-link <?= $cur==='dashboard.php'?'active':'' ?>">Inicio</a>
            <a href="/student/nueva-solicitud.php" class="sbar-link <?= $cur==='nueva-solicitud.php'?'active':'' ?>">Nueva Solicitud</a>
            <a href="/student/mis-solicitudes.php" class="sbar-link <?= $cur==='mis-solicitudes.php'?'active':'' ?>">Mis Solicitudes</a>
        </nav>
